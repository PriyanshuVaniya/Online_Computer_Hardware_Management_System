				...
				<div class="table-responsive">
					<table class="timetable_sub">
						...
						<tbody>
                            ...
                            $no = 1;
                            $grandtotal = 0;
                            $q = mysqli_query($connection,"SELECT * FROM tbl_order where user_id='{$uid}'");
                            while($row = mysqli_fetch_array($q))
                            {
                                $oq = mysqli_query($connection,"SELECT * FROM tbl_orderdetail where order_id='{$row['order_id']}'");
                                $odata = mysqli_fetch_array($oq);
                                $pq = mysqli_query($connection,"SELECT * FROM tbl_product where product_id= '{$odata['product_id']}'");
							...
							<tr class="rem1">
								...
								<td class="invert-image">
									<a href="single.html">
										<img src="upload/<?php echo $data["image_path"];?>" class="img-responsive" height="50px"  width="200px">
									</a>
								</td>
									...
									<div class="quantity">
										<div class="quantity-select">
											<div class="entry value">
												<span> <?php echo $odata['quantity'] ?> </span>
											</div>
								...
								<td class="invert">&#8377;<?php echo $data["price"];?></td>
                                <td class="invert">
                                    <?php 
                                        $subtotal = $odata['quantity'] * $data["price"];
                                        $grandtotal += $subtotal;
                                        echo '&#8377;' . $subtotal;
                                    ?>
                                </td>
                                <?php
                                $dq = mysqli_query($connection,"SELECT * FROM tbl_delivery where order_id= '{$row['order_id']}'");

while($dil = mysqli_fetch_array($dq)){
    ?>
								<td class="invert">
                                    <?php 
                                        echo $dil['delivery_status'];
                                    ?>
                                </td>
							</tr>
					...
					<a href='checkout.php'  class="btn btn-style btn-style-secondary mt-3">Checkout</a>
